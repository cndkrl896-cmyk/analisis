...
```{r, eval=FALSE}
vector_char <- c("rojo", "azul", "verde", "amarillo", "blanco", "negro",
                 "gris", "rosa", "naranja", "morado", "dorado",
                 "plateado", "café", "beige", "turquesa")
print(vector_char)
```
...
```{r, eval=FALSE}
vector_enteros <- c(5, 10, 15, 20, 25, 30, 35, 40, 45, 50)
...
```{r, eval=FALSE}
nombres <- c("Crystal", "Juan", "María", "Geovani", "Sofía",
             "Jose", "Aurora", "Melany", "Cindy", "Fernanda")
print(nombres)
```
...
```{r, eval=FALSE}
resultado <- sqrt(2 * pi) / max(3, 2, 4, 1, 8)
print(resultado)
```
...
```{r, eval=FALSE}
secuencia <- seq(0, pi, length.out = 10)
print(secuencia)
```
...
```{r}
limite <- log2(8)
secuencia <- seq(0, limite, length.out = 20)
print(secuencia)
